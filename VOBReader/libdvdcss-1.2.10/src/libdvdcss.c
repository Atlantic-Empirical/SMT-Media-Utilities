...
LIBDVDCSS_EXPORT int dvdcss_seek ( dvdcss_t dvdcss, int i_blocks, int i_flags )
{
    ...
    if( 
		( ( i_flags & DVDCSS_SEEK_MPEG ) && ( dvdcss->i_method != DVDCSS_METHOD_TITLE ) ) 
		|| 
 ...
 */
LIBDVDCSS_EXPORT int dvdcss_read ( dvdcss_t dvdcss, void *p_buffer, int i_blocks, int i_flags )
{
    int i_ret, i_index;
...
// * This function returns whether the DVD is scrambled.
// */
LIBDVDCSS_EXPORT int drive_unlock ( char * DriveLetter )
{
    return _drive_unlock(DriveLetter);
}